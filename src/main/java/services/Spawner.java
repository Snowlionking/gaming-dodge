...
public class Spawner {
	...
	public void spawnBasicEnemy(Handler handler) {
		handler.addEnemy(new BasicEnemy(r.nextInt(GameModel.WIDTH - 120), r.nextInt(GameModel.HEIGHT - 120), Id.BASIC_ENEMY));
	}
	
	public void spawnFastEnemy(Handler handler) {
		handler.addEnemy(new FastEnemy(r.nextInt(GameModel.WIDTH - 120), r.nextInt(GameModel.HEIGHT - 120), Id.FAST_ENEMY));
	}

	public void spawnTrackEnemy(Handler handler, Player player) {
		handler.setTrackEnemy(new TrackEnemy(r.nextInt(GameModel.WIDTH - 120), r.nextInt(GameModel.HEIGHT - 120), Id.TRACK_ENEMY, player));
	}
	...
	public void spawnPowerUp(Handler handler) {
		handler.addPowerUp(new PowerUp(r.nextInt(GameModel.WIDTH - 120),  r.nextInt(GameModel.HEIGHT - 120), Id.POWERUP, PowerUps.INVINCIBLE));
	}
